...
type Site = { id: string; name: string; latitude: number; longitude: number; description?: string | null; source: "OSM" | "COMMUNITY"; sourceUrl?: string | null; reviews: Array<{ id: string; rating: number; comment?: string | null; createdAt: string; photos: { id: string }[]; user: { publicId: string | null; username: string | null } }> };
...
export function SiteDetailFeature({ siteId }: { siteId: string }) {
  ...
  const point: MapPoint[] = [{ id: site.id, type: "site", siteName: site.name, latitude: site.latitude, longitude: site.longitude, description: site.description, reviewCount: site.reviews.length }];
  ...
  return <div className="feature-page site-detail-page"><header className="site-detail-hero"><div><span>{c.diveSite} · {site.source}</span><h1>{site.name}</h1><p><MapPin size={15} />{site.latitude.toFixed(5)}, {site.longitude.toFixed(5)}</p>{average != null && <strong>★ {average.toFixed(1)} · {site.reviews.length} {c.reviewCount.toLowerCase()}</strong>}</div>{site.sourceUrl && <a href={site.sourceUrl} target="_blank" rel="noreferrer">OpenStreetMap <ExternalLink size={14} /></a>}</header>
    ...
    <small className="site-data-credit">{c.siteDataCredit}</small>
  </div>;
}
