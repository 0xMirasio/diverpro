...
type Site = { id: string; name: string; latitude: number; longitude: number; description?: string | null; source: "OSM" | "OPEN_DIVEMAP" | "EMODNET_WRECK" | "COMMUNITY"; sourceUrl?: string | null; countryName?: string | null; seaName?: string | null; environment?: string | null; topologies?: string[] | null; maxDepthM?: number | null; entryType?: string | null; metadata?: { openDiveMap?: unknown; emodnetWreck?: unknown } | null; reviews: Array<{ id: string; rating: number; comment?: string | null; createdAt: string; photos: { id: string }[]; user: { publicId: string | null; username: string | null } }> };
...
export function SiteDetailFeature({ siteId }: { siteId: string }) {
  ...
  const sourceName = site.source === "OPEN_DIVEMAP" ? "OpenDiveMap" : site.source === "OSM" ? "OpenStreetMap" : site.source === "EMODNET_WRECK" ? "EMODnet / UKHO" : "BlueMates";
